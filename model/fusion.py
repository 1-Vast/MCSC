"""Shared-space and cross-modal fusion blocks for PRISM."""
...
class SharedInteractionSpace(nn.Module):
    """Seed the shared interaction token from drug and target tokens."""
    ...
    def forward(
        ...
        seed = self.norm(fused + self.delta(joined))
        return seed.unsqueeze(1)


class GatedCrossModalAttentionBlock(nn.Module):
